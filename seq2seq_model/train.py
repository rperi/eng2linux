...
if __name__ == '__main__':
    ...
    trainIters(encoder1, attn_decoder1,  repeat=iters, print_every=100)

    enc_dec = [encoder1, attn_decoder1]
    evaluateRandomly(encoder1, attn_decoder1)
    
    dtime = datetime.now().strftime("%I_%M%p_%B_%d_%Y")
    file_name = '_iter%s_TeacherForcing1_stop_merged.pickle' % str(iters)
    with open('enc_dec_'+ dtime + file_name, 'wb') as handle:

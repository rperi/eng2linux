...
def augment_eng2linux_Data(data_file, out_data_file, max_num_syn, FLAG_POS_Stanf, FLAG_Save_Verbs2Syns_dict):
    ...
    if FLAG_POS_Stanf == 0:
        ...
        CWD = os.getcwd()
        os.system("wget -N https://nlp.stanford.edu/software/stanford-postagger-full-2017-06-09.zip") #downloads zip file only if it doesn't exist into current directory

        # extracts zip file only if it doesn't exist into current directory
        if not os.path.isdir(CWD + "/stanford-postagger-full-2017-06-09"):
            zip = zipfile.ZipFile('stanford-postagger-full-2017-06-09.zip')
            zip.extractall()

        POS_Tagger_Stanford_dir  = CWD + "/stanford-postagger-full-2017-06-09"
    ...
    for idx, lines in enumerate(data.readlines()):
        ...
        # The current version supports only replacing a maximum of 2 verbs per description
        if verbs_in_desc:
...
if __name__ == '__main__':
    ...
    # Parameters
    data_file = "/home/raghuveer/coursework/CSCI662/final_project/data/data_TJ.txt"  # file containing the original command-description pairs
    out_data_file = "/home/raghuveer/coursework/CSCI662/final_project/data/data_augmented.txt"  # output file where the new augmented pairs are written
